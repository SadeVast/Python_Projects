...
WIDH =shutil.get_terminal_size()[0]
WIDH -= 1
...
def main():
    ...
    ducklingLanes = [None] * (WIDH // DUCKLING_WIDTH)

    while True:
        for laneNum, ducklinObj in enumerate (ducklingLanes):
            if (ducklinObj == None and random.random() <= DENSITY):
                ducklinObj = Duckling()
                ducklingLanes[laneNum] = ducklinObj

            if ducklinObj != None:
                print(ducklinObj.getNextBodyPart(), end='')
                if ducklinObj.partToDisplayNext == None:
